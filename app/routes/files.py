...
from app.services.file_service import process_xlsx, process_docx
        ...
        with zipfile.ZipFile(BytesIO(content)) as zip_ref:
            ...
            for filename, file_info in file_list:
                try:
                    ...
                    # ✅ ИСПРАВЛЕНО ЗДЕСЬ: Добавлены скобки для кортежа
                        ...
                        with open(extract_path, "rb") as f:
                            ...
                            results.append(result)
                    # Файлы без нужного расширения просто игнорируются
