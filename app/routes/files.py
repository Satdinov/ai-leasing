...
from app.services.file_service import process_delta_report, process_docx, process_xlsx
...
async def upload_docx(file: UploadFile = File(...), db: Session = Depends(get_db)):
    if not file.filename.lower().endswith((".docx", ".doc")):
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail="Поддерживаются только файлы .docx и .doc",
        )
...
async def upload_folder(
    zip_file: UploadFile = File(...), db: Session = Depends(get_db)
):
    ...
    try:
        ...
        with zipfile.ZipFile(BytesIO(content)) as zip_ref:
            ...
            for file_info in zip_ref.infolist():
                ...
                if (
                    not corrected_filename.startswith("__MACOSX/")
                    and not corrected_filename.startswith("._")
                    and not corrected_filename.endswith("/")
                ):
            ...
            for filename, file_info in file_list:
                try:
                    ...
                    results.append(
                        {"status": "error", "filename": filename, "error": str(e)}
                    )
...
async def get_file_metadata(
    path: str = None, file_type: str = None, db: Session = Depends(get_db)
):
...
async def search_documents(query: str, path: str = None):
    ...
    results = vectorstore.similarity_search(
        query, k=5, filter=filters, collection_name="docx_data"
    )
    return [
        {
...
@router.post("/process-delta-report")
async def process_delta_report_route(
    file: UploadFile = File(...), db: Session = Depends(get_db)
):
    """
    Принимает DOCX отчет от Дельты, очищает его и добавляет в векторную базу.
    """
    if not file.filename.lower().endswith(".docx"):
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail="Поддерживаются только .docx файлы для отчетов Дельты.",
        )
    return await process_delta_report(file, db)
